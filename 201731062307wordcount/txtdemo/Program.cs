...
namespace txtdemo
{
    class Program
    {
        ...
        */
        ...
        {
            var result = from pair in nary orderby pair.Value descending select pair;
        ...
        static void compare(Dictionary<string,int>nary)
        {
            ...
            //for (int k = 1; k < nary.Count; k++)
            //{
            //    tmp = word[k];
            //    valueTmp = value[k];//次数
            //    int j = k;
            //    while (j > 0 && valueTmp > value[j-1])
            //    {k
            //        word[j] = word[j - 1];
            //        j--;
            //    }
            ...
            {
                for (int j = 1; j < nary.Count - k - 1; j++)
                {
                    if (value[k] == value[j])
                    {
